...
export function recommendShowcasePrices({state}){
  ...
  for(const [key, city] of state.city.entries()){
    ...
    // Базовая цель — от текущей или от нижней границы бэнда
    let target = current.price ?? minPrice;
...
export function recommendBuyPrices({state, sellRecos}){
  const recos = [];
  // получаем текущие наши buy-прайсы из shops по me
  ...
  for(const [key, city] of state.city.entries()){
    ...
    // Строгое требование: кандидат должен обеспечить прибыль ≥1 даже при продаже ГОРОДУ (capByCity)
    if(candidate!=null && candidate <= capByCity){
      targetBuy = candidate;
    }else{
      // иначе ставим максимум, который гарантирует ≥1 к городу или витрине (profitCapAny), но не выше capByCity если он валиден
